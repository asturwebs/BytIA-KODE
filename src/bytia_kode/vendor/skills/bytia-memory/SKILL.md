...
name: bytia-memory
description: >
  claude-mem v10.6.1 3-Layer workflow for semantic memory retrieval (O13).
...
allowed-tools: mcp__plugin_claude-mem_mcp-search__search, mcp__plugin_claude-mem_mcp-search__timeline, mcp__plugin_claude-mem_mcp-search__get_observations, Read
---
...
2. timeline(anchor=1525, depth_before=2, depth_after=2)
   → IDs contextuales: [1523, 1524, 1525, 1526, 1527]

3. get_observations(ids=[1523, 1524, 1525, 1526, 1527])
   → Detalles completos (~2,500 tokens)
```
...
└─ Amplio? ("en qué trabajamos")
    └─ search(query amplia, limit=10) → timeline varias anclas → get_observations filtrados
```
